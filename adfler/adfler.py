...
def main():
    parser = argparse.ArgumentParser(description="Advanced Drone Flight Log Entity Recognizer (ADFLER)")
    
    # Global arguments
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument("--output", help="Output directory")
    parser.add_argument("--evidence", help="Evidence directory")
    parser.add_argument("--model", help="Model directory or HuggingFace model ID (default: swardiantara/ADFLER-albert-base-v2)")
    ...
    if 'model_dir' not in config:
        config['model_dir'] = "swardiantara/ADFLER-albert-base-v2"
        ...
        if construct_timeline(config):
            if run_ner(config):
                if run_report(config):
                    print("\nADFLER Pipeline completed successfully.")
                else:
                    print("\nADFLER Pipeline failed at report generation.")
            else:
                print("\nADFLER Pipeline failed at Entity Recognition.")
        else:
            print("\nADFLER Pipeline failed at Forensic timeline construction.")
    else:
        print("\nADFLER Pipeline failed at Evidence checking.")
